...
static void term_osc_cb(struct tsm_vte *vte, const char *osc,
			size_t osc_len, void *data)
{
	terminal_t* terminal = (terminal_t*)data;

	if (strncmp(osc, "image:", 6) == 0)
		term_esc_show_image(terminal, (char *)osc + 6);
	else if (strncmp(osc, "box:", 4) == 0)
		term_esc_draw_box(terminal, (char *)osc + 4);
	else if (strncmp(osc, "input:", 6) == 0)
		term_esc_input(terminal, (char *)osc + 6);
	else if (strncmp(osc, "switchvt:", 9) == 0)
		term_esc_switchvt(terminal, (char *)osc + 9);
	else if (strncmp(osc, "drmdropmaster", 13) == 0)
		term_esc_drmdropmaster(terminal, (char *)osc + 13);
	else if (is_xterm_osc((char *)osc))
		; /* Ignore it. */
	else
